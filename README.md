...
```
## Design 
Thanks to pyfirmata, I can program the code with python. The only thing I need to do is upload the standarfirmata to arduino. Once that is done, I can write code in serial. Arduino becomes a dummy element that read code and executes them. It controls the two servos for pan and tilt and has a pointer to show where it is pointing, and at this stage, I thought to use two solenoids to control the spray and the pump. If I use only one solenoid, the time travel from the pressurized bucket to the end of the nozzle would be longer. There are at least 1.5 meters in the distance between the two points requiring a more extended period for the ink to reach the end of the nozzle. Instead, if I place one at the beginning of the pump and another at the end at the nozzle, the time travel from the two points would be shorter; due to opening the first solenoid first and, sleep for 0.05 sec, opening the other so the squirt would be more robust and always ready. 
### Robot
Initially, the idea was to place two light stands and a bar on top. Place the robot in the middle at 1.7 meters, pointing to the floor. So, put a canvas down and let it spray from there to have a clean shot and drip-free effect. But the mechanism is more accessible than I thought, so it is not stable, leading to inaccuracy. The previous version was to place the robot on a tripod, which resulted in optimal for the stability but less for canvas effect and style. 
## Conclusion
From the last iteration, despite the negative value seems to be more accurate, but not enught to draw with the robot. My guess is that I need to do a conversion from pixel to real life points and get a more accurate calibration. In computer vision there are many way how to achieve this one is to apply solvePnP that requires the pixel coordinates (x,y), and also a depth (z) corresponding world coordinates (x,y,z) a so called matrix K to calibrate the the camera, usually is required a checkerbox or an [aruco checkerbox](https://github.com/riccardogiorato/aruco_checkerboard). If camera is not calibrated can have a margin error of 5 to 30 cm. An extrinsic matrix used to transform matrix from the world coordinate system 3D to the camera/pixel coordinate system 2D. My problem is the conversion from pixel coordinate points p (2D) + depth d to world coordinate points X (3D). I could not work out a solution. there are many example using this formula 
```
